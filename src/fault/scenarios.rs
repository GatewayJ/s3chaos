...
pub const FAULT_SCENARIO_CATALOG: &[FaultScenarioSpec] = &[
    ...
    FaultScenarioSpec {
        ...
        description: "Planned scenario-owned RustFS admin flow: decommission proven pool ID 1 under continuous S3 workload in a fresh multi-pool Tenant.",
        priority: FaultPriority::P1,
        backend: FaultBackend::PlannedReliabilityWorkflow,
        status: FaultScenarioStatus::Planned,
        workload_profile: FaultScenarioWorkloadProfile::Default,
        ...
        ci_phase: "planned",
        target: "RustFS pool ID 1 and its exact runtime expression inside a fresh two-pool fault-test Tenant",
        target_proof: &[
            "admin-topology-proof.json must bind the fresh Tenant UID and typed pools to unique RustFS runtime pool IDs and expressions",
            "preflight must prove healthy idle pools, no concurrent decommission/rebalance, and enough remaining free capacity for target used bytes",
            "admin operation evidence must bind start/status requests to exact pool ID 1 with by-id=true",
        ],
        validation: "decommission reaches successful completion without failed moves or cancellation, before/after topology proves the target absent or terminal, and continuous S3 history/checker evidence preserves the committed object model",
    ...
    FaultScenarioSpec {
        ...
        description: "Planned scenario-owned RustFS admin flow: run rebalance under continuous S3 workload in a fresh multi-pool Tenant.",
        priority: FaultPriority::P1,
        backend: FaultBackend::PlannedReliabilityWorkflow,
        status: FaultScenarioStatus::Planned,
        workload_profile: FaultScenarioWorkloadProfile::Default,
        ...
        target_proof: &[
            "admin-topology-proof.json must bind the fresh Tenant UID and typed pools to unique RustFS runtime pool IDs and expressions",
            "preflight must prove healthy idle pools and no concurrent decommission/rebalance",
            "admin operation evidence must bind the non-empty start operation ID to status and progress observations",
        ],
        validation: "every rebalance pool reaches successful completion without stop, error, or cleanup warning, pool identities remain stable before/after, and continuous S3 history/checker evidence preserves the committed object model",
        observability: "admin-topology-proof.json, admin-operation.json, monotonic admin-operation-progress.jsonl, workload history, checker reports, RustFS logs",
