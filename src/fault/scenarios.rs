...
pub const FAULT_SCENARIO_CATALOG: &[FaultScenarioSpec] = &[
    ...
    FaultScenarioSpec {
        ...
        validation: "the crash window contains an acknowledged versioned mutation while drop_writes is active; after the crash boundary the node stays down for at least the minimum hold while every prefilled object reads back with its committed bytes and a fresh PUT/GET/LIST/DELETE/multipart probe succeeds through a surviving server; after the healthy table is restored, the filesystem check passes, RustFS reports every drive ok and every Pod ready, fresh post-recovery writes succeed, and every committed version is re-read; because one EC volume is lost this remains a durability negative-control proxy, while the node-down availability contract is a real gate",
