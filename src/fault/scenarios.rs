...
pub const FAULT_SCENARIO_CATALOG: &[FaultScenarioSpec] = &[
    ...
    FaultScenarioSpec {
        ...
        description: "Planned scenario-owned RustFS admin flow: decommission the named, runtime-bound target pool under a bounded continuous S3 workload in a fresh multi-pool Tenant.",
        priority: FaultPriority::P1,
        backend: FaultBackend::PlannedReliabilityWorkflow,
        status: FaultScenarioStatus::Planned,
        workload_profile: FaultScenarioWorkloadProfile::Default,
        ...
        target_proof: &[
            ...
            "admin operation evidence must bind run ID, case, Tenant UID, exact start/status requests, progress, and successful pre-start/post-terminal pool lists to the resolved target pool with by-id=true",
        ],
    ...
    FaultScenarioSpec {
        ...
        target_proof: &[
            ...
            "admin operation evidence must bind run ID, case, Tenant UID, and the non-empty start operation ID to exact status/progress observations and successful pre-start/post-terminal pool lists with at least one participating pool and positive movement",
        ],
        validation: "every participating rebalance pool reaches successful completion, nonparticipants remain terminal, no stop/error/cleanup warning occurs, pool identities remain stable before/after, and continuous S3 history/checker evidence preserves the committed object model",
        observability: "admin-topology-proof.json, admin-operation.json, monotonic admin-operation-progress.jsonl, workload history, checker reports, RustFS logs",
