...
pub(crate) trait ProtocolTimeoutPolicy: Sync {
    async fn wait_for_wave(&self, wave_index: usize, started_at_millis: u128) -> Result<()>;
}
    ...
    pub(crate) async fn execute(
        ...
        if let Some(reason) = preflight_failure {
            ...
            for wave_index in 0..wave_count {
                ...
                let futures = wave.iter().map(|case| self.case_lifecycle.run_case(case));
                ...
                let wave_result = tokio::select! {
                    ...
                    timeout = self.timeout.wait_for_wave(wave_index, started_at_millis) => {
                        Err((ExecutionControl::Timeout, timeout))
                    },
                };
                match wave_result {
                    Ok(results) => {
                    ...
                    Err((control, outcome)) => {
                        stop_reason = Some(match (control, outcome) {
                            ...
                            (ExecutionControl::Timeout, Ok(())) => format!(
                                "protocol suite wave {wave_index} timed out; cleanup requested"
                            ),
                            (ExecutionControl::Timeout, Err(error)) => {
                                format!("protocol timeout handler failed: {error}")
                            }
                        });
                        for case in wave {
                            let cleanup = self
                            ...
                            cases.push(ExecutedProtocolCase {
                                execution: ProtocolCaseExecution::interrupted(&case.id),
                                cleanup,
                            });
                        }
                    }
...
mod tests {
    ...
    #[derive(Default)]
    struct ImmediateTimeout(Arc<Mutex<Vec<(usize, u128)>>>);

    #[async_trait]
    impl ProtocolTimeoutPolicy for ImmediateTimeout {
        async fn wait_for_wave(&self, wave_index: usize, started_at_millis: u128) -> Result<()> {
            self.0
                .lock()
                .expect("timeout calls")
                .push((wave_index, started_at_millis));
            Ok(())
        }
    }
    ...
    async fn injected_clock_and_timeout_use_safe_interruption_cleanup_path() {
        ...
        let cleanup = FakeCleanup::default();
        let timeout = ImmediateTimeout::default();
        ...
        assert_eq!(*timeout.0.lock().expect("timeout calls"), vec![(0, 42)]);
        assert_eq!(
            *cleanup.interrupted.lock().expect("interrupted"),
            vec!["first", "second"]
        );
