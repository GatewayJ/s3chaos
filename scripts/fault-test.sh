...
process_group_id() {
  local process="$1" group
  group="$(ps -o pgid= -p "$process" 2>/dev/null | tr -d '[:space:]')"
...
run_scenario() {
  ...
  test_pid=$!
  ACTIVE_PROCESS_GROUP="$test_pid"
  ACTIVE_PID="$test_pid"
  ACTIVE_PROCESS_GROUP="$(process_group_id "$test_pid" || printf '%s\n' "$test_pid")"
  health_checks=0
...
run_suite() {
  ...
  ACTIVE_PID="$!"
  ACTIVE_PROCESS_GROUP="$ACTIVE_PID"
  ACTIVE_PROCESS_GROUP="$(process_group_id "$ACTIVE_PID" || printf '%s\n' "$ACTIVE_PID")"
  health_checks=0
  consecutive_health_failures=0
